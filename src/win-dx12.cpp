...
namespace ClapSupport {
	...
	private:
		bool init() {
			...
			if (D3D12CreateDevice(nullptr, featureLevel, IID_PPV_ARGS(&_device)) != S_OK) return false;
	...
	public:
		DX12Context(imgui_clap_editor *editor, HWND windowHandleParent) : _editor(editor) {
			...
			_windowHandle = ::CreateWindow(_windowClass.lpszClassName, _T("ImGui Clap Saw Demo"), WS_CHILD | WS_VISIBLE,
			...
			ImGui_ImplDX12_Init(globals._device, DX12Globals::numOfFramesInFlight, DXGI_FORMAT_R8G8B8A8_UNORM,
								_srvDescHeap, _srvDescHeap->GetCPUDescriptorHandleForHeapStart(),
		...
		void render() {
			...
			_commandList->ClearRenderTargetView(_mainRenderTargetDescriptor[backBufferIdx], clear_color_with_alpha, 0,
												nullptr);
			_commandList->OMSetRenderTargets(1, &_mainRenderTargetDescriptor[backBufferIdx], FALSE, nullptr);
		...
		bool resizeWindow(int width, int height) {
			if (_windowHandle == nullptr) return false;
	...
	private:
		bool createHeapDescriptors() {
			...
			rtvDesc.NodeMask = 1;
			if (globals._device->CreateDescriptorHeap(&rtvDesc, IID_PPV_ARGS(&_rtvDescHeap)) != S_OK) return false;
			...
			if (globals._device->CreateDescriptorHeap(&srvDesc, IID_PPV_ARGS(&_srvDescHeap)) != S_OK) return false;
		...
		bool createCommandListAndQueue() {
			...
			desc.NodeMask = 1;
			if (globals._device->CreateCommandQueue(&desc, IID_PPV_ARGS(&_commandQueue)) != S_OK) return false;

			for (UINT i = 0; i < DX12Globals::numOfFramesInFlight; i++)
				if (globals._device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
															IID_PPV_ARGS(&_frameContext[i].CommandAllocator)) != S_OK)
					return false;

			if (globals._device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, _frameContext[0].CommandAllocator,
												   nullptr, IID_PPV_ARGS(&_commandList)) != S_OK ||
		...
		bool createFences() {
			if (globals._device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&_fence)) != S_OK) return false;
		...
		bool createSwapChain(HWND windowHandle) {
			...
			if (dxgiFactory->CreateSwapChainForHwnd(_commandQueue, windowHandle, &sd, nullptr, nullptr, &swapChain1) !=
				S_OK)
				return false;
			if (swapChain1->QueryInterface(IID_PPV_ARGS(&_swapChain)) != S_OK) return false;
			swapChain1->Release();
			dxgiFactory->Release();
		...
		void cleanup() {
			...
			if (_swapChainWaitableObject != nullptr) { ::CloseHandle(_swapChainWaitableObject); }
			for (UINT i = 0; i < DX12Globals::numOfFramesInFlight; i++)
	...
	LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		if (dx12context) dx12context->setImGuiContext();

		switch (msg) {
		case WM_MOUSEMOVE: {
			auto focus = ::GetFocus();
			if (focus != hWnd && dx12context->getWindowHandle() == hWnd) { ::SetFocus(hWnd); }
		} break;
		...
		case WM_SYSCOMMAND:
			if ((wParam & 0xfff0) == SC_KEYMENU) // Disable ALT application menu
				return 0;
			break;
		case WM_TIMER: dx12context->render(); return 0;
		default: break;
		}
...
bool imgui_clap_guiCreateWith(imgui_clap_editor *e, const clap_host_timer_support_t *) {
	IMGUI_CHECKVERSION();
	e->onGuiCreate();
	return true;
}
void imgui_clap_guiDestroyWith(imgui_clap_editor *e, const clap_host_timer_support_t *) {
	e->onGuiDestroy();
...
bool imgui_clap_guiSetSizeWith(imgui_clap_editor *e, int width, int height) {
	if (auto context = (ClapSupport::DX12Context *)e->ctx) { return context->resizeWindow(width, height); }

	return false;
}

bool imgui_clap_guiSetWindowTitleWith(imgui_clap_editor *e, const char *t) {
	if (auto context = (ClapSupport::DX12Context *)e->ctx) {
		return SetWindowTextA(context->getWindowHandle(), (LPCSTR)t);
	}
